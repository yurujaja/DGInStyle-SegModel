...
# Modifications:
# - Remove share src backward, use separate loss
import math
import os
import random
...
class DACS(UDADecorator):
    ...
    def forward_train(self,
        ...
        if self.style_consistency_lambda > 0 and self.get_model().backbone.style_hallucination is not None:
        ...
        if self.enable_fdist:
            ...
            # Seperate hrda loss backward
            if isinstance(self.get_model(), HRDAEncoderDecoder):
                clean_loss_lr = clean_losses['decode.loss_seg']
                clean_loss_hr = clean_losses['decode.hr.loss_seg']
                clean_loss_lr.backward(retain_graph=True)
                clean_loss_hr.backward(retain_graph=True)
        ...
        elif self.style_consistency_lambda > 0:
            clean_loss.backward(retain_graph=True)
        else:
            clean_loss.backward()
        # Style Consistency
